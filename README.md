...
Produces a statically linked `infix-webui` binary with all templates,
CSS, and JS embedded.
...
```sh
./infix-webui -restconf https://192.168.0.1/restconf -listen :8080
```
...
This runs `go run .` on port 8080 with live recompilation on each
